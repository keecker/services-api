 ...
 *
 * This is a copy of android.support.test.rule.ServiceTestRule with a fix to handle reuse of the
 * rule accross test.
 *
 * The original code only use a single Countdownlatch for one instance of ServiceTestRule, which
 * means it was not possible to reuse an instance accross multiple test.
 *
 * Samples of android.support.test.rule.ServiceTestRule seems to indicate that the original class
 * should be reused on multiple tests so it's possible that this will be fixes in a new version
 * of the support library.
 *
 * TODO: (Romain) We should create a test library and move this class in the library.
 *
 */
public class ServiceTestRule implements TestRule {
    ...
    boolean bindServiceAndWait(Intent intent, final ServiceConnection conn,
        ...
        if (isBound) {
            ...
            waitOnLatch(mServiceConn.mConnectedLatch, "connected");
        } else {
    ...
    static class ProxyServiceConnection implements ServiceConnection {
        ...
        // We use field variable which resets the CountDownLatch for each new ServiceConnection
